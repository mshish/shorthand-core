...
2. **No `cwd` is passed to the agent at all.** The option is spread in conditionally, so the
   agent SDK never receives a working directory rather than receiving a misleading one.
